...
fn main_loop(mut entries: Vec<Entry>, config: Config) -> io::Result<()> {
    ...
    loop {
        ...
        if redraw {
            ...
            draw_screen(&filtered_entries, current_item, &mut first_entry, &search, &config, &mode, &entry_path, &edit_details, edit_field, edit_cursor_pos, &series, &mut series_selection, &new_series, season_number)?;
            redraw = false;
        }
        ...
        if event::poll(Duration::from_millis(100))? {
            if let Event::Key(KeyEvent { code, modifiers, .. }) = event::read()? {
                match mode {
                    ...
                    Mode::Edit => {
                        handlers::handle_edit_mode(code, modifiers, current_item, &mut filtered_entries, &mut edit_details, &mut season_number, &mut entries, &mut mode, &mut edit_field, &mut edit_cursor_pos, &mut redraw);
                    }
                    Mode::Browse => {
                        if !handlers::handle_browse_mode(code, modifiers, &mut current_item, &mut first_entry, &mut filtered_entries, &mut entries, &mut search, &mut playing_file, &mut mode, &mut edit_details, &mut season_number, &mut redraw, &config, &tx)? {
                            break Ok(());
                        }
                    }
                    Mode::SeriesSelect => {
